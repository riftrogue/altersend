...
# Architecture: honour ARCH env override (set by per-arch npm scripts);
# otherwise detect from the host so a bare invocation still works.
if [ -n "${ARCH:-}" ]; then
  case "$ARCH" in
    x64 | arm64) ;;
    *) echo "Unsupported ARCH override: $ARCH (expected x64 or arm64)"; exit 1 ;;
  esac
else
  UNAME_ARCH=$(uname -m)
  case "$UNAME_ARCH" in
...
OUTPUT="$ROOT/out/make/${APP_NAME}-${ARCH}.AppImage"
